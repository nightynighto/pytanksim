...
def d_ads(z, eps0, beta, lam, gam, T, dB, d0, dvap, dliq):
    potential = chem(T, dB) + DRA(z, eps0, beta, lam, gam, T)
    ##f is the function to be minimized to get adsorbate density
    def f(d_z):
        y = potential - chem(T, d_z)
        return y
    ...
    if potential > chem(T, dvap) and d0<dliq:
        d0 = dliq * 1.1
    return sp.optimize.fsolve(f,d0)[0]


def N_ex(eps0, beta, lam, gam, T, dB):
    d0 = dB
    z0 = lam + gam * T
    N = 50
    delta = z0/N
    ...
    if T < CP.CoolProp.PropsSI("Tcrit", "Hydrogen"):
        ...
        dliq = fluid.rhomolar()
         ...
         dliq = dvap
    for i in range(0,N): 
        d_z = d_ads(z0 - i*delta - delta/2, eps0, beta, lam, gam, T, dB, d0, dvap, dliq)
        integral += d_z * delta
        d0 = d_z
    return integral - dB * z0

def N_abs(eps0, beta, lam, gam, T, dB):
    d0 = dB
    z0 = lam + gam * T
    N = 50
    delta = z0/N
...
def fit_penalty(params, dataD, dataAd, dataT):
    ...
    for i in range(0, len(dataD)):
        difference.append(N_ex(eps0, beta, lam, gam, dataT[i], dataD[i]) - dataAd[i])
    return difference
...
def n_exc_PT(eps0, beta, lam, gam, quality = 1):
    def na(p, T):
        if p == 0:
            return 0
        ...
        if T < fluid.T_critical():
            ...
            fluid.update(CP.PT_INPUTS, p, T)
            bulkdens = fluid.rhomolar()
        return N_ex(eps0, beta, lam, gam, T, bulkdens)
    return na

def n_abs_PT(eps0, beta, lam, gam, quality = 1):
    def na(p, T):
        if p == 0:
            return 0
        elif p > fluid.pmax()/10:
            pres = fluid.pmax()/10
            fluid.update(CP.PT_INPUTS, pres, T)
            bulkdens = fluid.rhomolar()
            return N_abs(eps0, beta, lam, gam, T, bulkdens)
        ##Need to add an exception near saturation to ensure the density value used is same as the selected phase
        if T < fluid.T_critical():
            fluid.update(CP.QT_INPUTS, quality, T)
            psaturation = fluid.p()
